...
freq = 10^-3;
% Attribuisco a ciascun input un segnale; in questo caso abbiamo 3 sinusoidi
% della medesima frequenza ed ampiezza
standard_signal = sin(freq*t);
u = [standard_signal; standard_signal; standard_signal];


y = lsim(sys, u, t);
subplot(2,1,1)
plot(t, y);
title("Segnale amplificato (frequenza bassa)")
hold on
plot(t, sin(freq*t))
ylabel("Uscita (somma degli input)")
legend("frequenza amplificata: 10^{-3}", "Segnale iniziale")
grid
...
plot(t, sin(freq*t))
ylabel("Uscita (somma degli input)")
title("Segnale attenuato (frequenza alta)")
legend("frequenza attenuata: 10^{-2}", "Segnale iniziale")
grid
